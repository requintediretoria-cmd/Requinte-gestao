...
export default function NovaCompraButton() {
  ...
  const suggestCategoria = useCallback(async (fornecedor: string, produto: string) => {
    ...
    try {
      ...
      if (json.categoria) setForm((p) => ({ ...p, categoria: json.categoria }));
    } finally {
      setSugg(false);
    }
  }, []);
  ...
  return (
    <>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            <div className="px-6 py-5 border-b border-gray-100 flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-0.5">Pagamento à vista em dinheiro</p>
              </div>
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <input className="input" placeholder="Ex: Hortifruti Central"
                  value={form.fornecedor} onChange={(e) => set("fornecedor", e.target.value)} />
              </div>

              <div>
                <label className="label">Produto *</label>
                <input className="input" placeholder="Ex: Tomate, Frango, Detergente..."
                  value={form.produto}
                  onChange={(e) => set("produto", e.target.value)}
                  onBlur={() => suggestCategoria(form.fornecedor, form.produto)} />
                <p className="text-xs text-gray-400 mt-1">Categoria sugerida automaticamente ao sair do campo</p>
              </div>

              <div>
                <label className="label">
                  Categoria
                  {suggesting && <span className="ml-2 text-brand-500 font-normal animate-pulse">sugerindo...</span>}
                </label>
                <select className="input" value={form.categoria} onChange={(e) => set("categoria", e.target.value)}>
                  <option value="">Aguardando sugestão...</option>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input className="input" type="date"
                    value={form.dataCompra} onChange={(e) => set("dataCompra", e.target.value)} />
                </div>
              </div>
            ...
            <div className="px-6 py-4 border-t border-gray-100 flex items-center justify-between">
              <span className="text-xs text-gray-400 flex items-center gap-1">💵 Dinheiro · à vista</span>
      ...
      <style>{`
        .label { display: flex; font-size: 0.7rem; font-weight: 500; color: #6b7280; margin-bottom: 4px; text-transform: uppercase; letter-spacing: 0.04em; gap: 6px; }
